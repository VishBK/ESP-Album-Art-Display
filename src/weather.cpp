...
void displayTodaysTempRange() {
  display->fillRect(TEMPRANGE_X, TEMPRANGE_Y, TEMPRANGE_WIDTH, TEMPRANGE_HEIGHT, 0);
  display->setTextSize(1);     // size 1 == 8 pixels high
  display->setTextWrap(false); // Don't wrap at end of line - will do ourselves
  display->setTextColor(TEMPRANGE_COLOR);

  display->setCursor(TEMPRANGE_X, TEMPRANGE_Y);   
  display->printf("%3d/%3d C", minTempToday, maxTempToday);
  
  // Draw the degree symbol manually
  display->fillRect(TEMPRANGE_X + 44, TEMPRANGE_Y, 2, 2, TEMPRANGE_COLOR);
}
...
void drawBitmap(int startx, int starty, int width, int height, uint32_t *bitmap) {
  ...
  for (int yy = 0; yy < height; yy++) {
    for (int xx = 0; xx < width; xx++) {
      display->drawPixel(startx+xx, starty+yy, color565(bitmap[counter]));
      counter++;
    }
  }
}
...
void drawBitmap(int startx, int starty, int width, int height, uint32_t *bitmap, bool enlarged) {
  ...
  if (enlarged) {
    for (int yy = 0; yy < height; yy++) {
      for (int xx = 0; xx < width; xx++) {
        display->drawPixel(startx+2*xx, starty+2*yy, color565(bitmap[counter]));
        display->drawPixel(startx+2*xx+1, starty+2*yy, color565(bitmap[counter]));
        display->drawPixel(startx+2*xx, starty+2*yy+1, color565(bitmap[counter]));
        display->drawPixel(startx+2*xx+1, starty+2*yy+1, color565(bitmap[counter]));
        counter++;
      }
    }
  }
...
void getAccuWeatherData() {
  ...
  ESP_LOGD("accuweather", "%i", minTempToday);
  ESP_LOGD("accuweather", "%i", maxTempToday);
